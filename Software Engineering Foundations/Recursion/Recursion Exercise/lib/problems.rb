# Recursion Exercise - From App Academy Open Full Stack Course
# https://open.appacademy.io/learn/full-stack-online/software-engineering-foundations/recursion-exercise
...
#     1-dimensional array: [1,[['a','b']],['c','d']] => [] + 
#     2-dimensional array: [['some data']]
#     3-dimensional array: [[['some data']]]
def flatten(data)
    return [data] if !data.instance_of?(Array)
    return data if data.instance_of?(Array) && data.none? { |el| el.instance_of?(Array) }
    idx = data.find_index { |el| el.instance_of?(Array) }
    flatten(data[0...idx] + data[idx] + data[idx+1..-1])
end